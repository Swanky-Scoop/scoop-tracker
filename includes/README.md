...
| `_policy.php` | `scoop_access_policy()` — role-keyed permission matrix (`administrator`, `editor`, `author`, `ice_cream_maker`, `_default`). `scoop_user_can_route()` and `scoop_user_writeable_fields()` gate every request. Also holds `scoop_create_custom_roles()` (activation hook, registers legacy `scoop_manager`/`scoop_backhouse`/`scoop_lead`/`scoop_staff` roles) — those roles/caps are not read by the matrix above and are currently unused by any request path. |
