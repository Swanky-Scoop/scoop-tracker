...
| `_policy.php` | `scoop_access_policy()` — role-keyed permission matrix (`administrator`, `editor`, `author`, `kitchen_manager`, `shift_lead`, `ice_cream_maker`, `kiosk`, `_default`). Role slugs are real WP roles created via the User Role Editor plugin, not registered by this codebase. `_default` is a deny-everything fallback for any unrecognized role — `scoop_get_user_policy()` checks named roles in priority order and only falls through to `_default` if none match. `scoop_user_can_route()` and `scoop_user_writeable_fields()` gate every request. (The legacy `scoop_create_custom_roles()`/`scoop_manager`/`scoop_backhouse`/`scoop_lead`/`scoop_staff` role-registration code that used to live here was dead — never read by the matrix — and has been removed.) |
...
| `tub-state.php` | Auto-sets `created_on` / `changed_on` on tub creation. Enforces the tub state machine (`Hardening` → `Opened` → `Emptied`). Used to also demote a tub to `post_status='draft'` on flipping to `Emptied` — removed (see `includes/republish-tubs-ui.php`'s header comment): most relationship fields across this plugin's Pods schema are `pick_post_status=publish`, so a draft tub silently dropped out of any *other* record's relationship to it (confirmed on `inventory_change.tubs`); `state='Emptied'` already keeps a tub out of the way everywhere this app looks, so the demotion was redundant and actively harmful. |
