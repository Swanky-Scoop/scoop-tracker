...
Other',
  ...
  // Slot designation fields (current/immediate/next flavor) — hand-authored,
  // fields-only on the EXISTING 'slot' pod (same pattern as 'tub' below).
  // These were the last save-hook family calling Pods with NO schema
  // declaration anywhere: every reader in includes/hooks/cabinet-slot.php
  // traverses them as "<field>.ID = …" (syntax Pods can only resolve against
  // a real pick relation) and the designation-uniqueness hooks write them,
  // so an environment missing them (or with them drifted to plain numbers by
  // a gen-pods run predating _specs.php's titleMap lines) 500s every slot
  // save the same way the missing flavor_request pod did. The hook code now
  // guards on that (scoop_slot_designation_schema_ready()) and degrades to a
  // logged skip — but guards only degrade; this declaration is what makes
  // "repaired via Scoop -> Schema Sync" actually possible. pick attrs mirror
  // the hand-authored tub.moving_to field; no 'tub' field is declared here —
  // slot.tub is the bidirectional sister of tub.slot (see change-tub.md) and
  // Schema Sync strips sister_id by design, so that pairing stays
  // hand-configured per environment.
  'slot' =>
  array (
    'fields' =>
    array (
      ...
      array (
        ...
        'object_storage_type' => 'post_type',
        'name' => 'current_flavor',
        'group' => ['name' => 'more_fields', 'pod' => 'slot'],
        'label' => 'Current flavor',
        'description' => 'The flavor currently in service in this slot. One of the three slot designations (see includes/hooks/cabinet-slot.php): at most one designation of a flavor may exist across the whole slot table, enforced by the dedupe hooks; also drives server-side demand (Debt board) and tub-moving earmarking.',
        'type' => 'pick',
        'pick_object' => 'post_type',
        'pick_val' => 'flavor',
        'pick_format_type' => 'single',
      ...
      array (
        ...
        'description' => 'The flavor replacing current as soon as a tub arrives (slot designations, see current_flavor). Same demand/moving machinery.',
        'type' => 'pick',
        'pick_object' => 'post_type',
        'pick_val' => 'flavor',
        'pick_format_type' => 'single',
      ...
      array (
        ...
        'group' => ['name' => 'more_fields', 'pod' => 'slot'],
        'label' => 'Next flavor',
        'description' => 'A further-out plan for this slot — scheduled here but not yet acted on (excluded from demand counting and tub-moving triggers, see includes/hooks/cabinet-slot.php).',
        'type' => 'pick',
        'pick_object' => 'post_type',
        'pick_val' => 'flavor',
        'pick_format_type' => 'single',
