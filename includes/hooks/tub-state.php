...
function scoop_enforce_tub_rules( $pieces, $is_new_item, $id = 0 ) {
  ...
  // Timestamps are always system-controlled — '__override__' is a real
  // pipeline state (see cabinet-workflow-grid-model.js, closeout.php) but no
  // longer an escape hatch from these rules. It used to bypass this whole
  // block so opened_on/emptied_at could be hand-edited by routing a save
  // through it; that path wasn't actually being used for that in practice,
  // so the bypass was dropped rather than kept as unused surface area.
