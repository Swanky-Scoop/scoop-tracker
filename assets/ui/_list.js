...
export default class List extends Dockable{
  ...
  // _bindPageStatusToggle (mirrors PageStatus freshness onto TOGGLE) now
  // lives on Dockable — shared with every non-List dockable view too. See
  // that file for the full comment.
  ...
  _attachCoreDom(){
    ...
    // Grid sets FRAME_HOST to a wrapper div around FRAME, only for
    // .action-target-docked grids (see grid.js's buildCoreDom); every other
    // grid — Tile always, most Grids too — has no such wrapper, so this
    // falls back to attaching FRAME itself, same as before.
  ...
  // _buildToggleButton/dockToggle/_setToggled/_syncDockHash/
  // DOCK_SLOT_SELECTORS/_closeSlotSiblings/_enforceCanvasExclusivity/
  // CANVAS_SINGLE_OPEN_BREAKPOINT/ICON_FONT_* all now live on Dockable —
  // shared with every non-List dockable view too (ShiftReportForm,
  // TaskForm). See that file for the full comments; List just inherits them
  // unchanged (this.target/_bindDockToggle() below wire it up the same way
  // a Dockable subclass would).
  ...
  async _bindEvents() {
    ...
    // Wires target._dockListInstance, data-canvas-mode, and the TOGGLE click
    // handler — see Dockable._bindDockToggle().
