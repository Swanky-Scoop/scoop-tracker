...
export default class ConfirmSwapModal extends El {
  ...
  _buildDom() {
    ...
    const BTN_GROUP = el('p', { classes: ['btn_group'] });
    BTN_GROUP.append(this.CONFIRM_BTN, this.OTHER_BTN);
  ...
  open(row) {
    this._row = row;
    this._selectedFlavorId = row.flavorId;
    this._preferWhole = true;
    this.PARTIAL_CHECK.checked = true;
    this._render();
  ...
  _render() {
    ...
    const tub = this.model.pickPromotableTub(this._selectedFlavorId, row.location, this._preferWhole);
  ...
  async _confirm() {
    ...
    const tubCells = { [tub.id]: { state: 'Opened' } };
    if (row.openTub?.id) tubCells[row.openTub.id] = { state: 'Emptied' };
    ...
    const rSlot = await this.api.postJson(
      { cells: { [row.slotId]: { current_flavor: this._selectedFlavorId, tubs: tub.id } } },
      'Cabinet',
    );
    if (!rSlot.ok || !rSlot.data?.ok) {
      alert(`Tubs were swapped, but the slot link failed to save.\n${rSlot?.data?.error ?? `HTTP ${rSlot?.status}`}\nUse Confirm Cabinet to fix the link.`);
      return;
    }
  ...
  async _confirmEmpty() {
    ...
    if (row.openTub?.id) {
      const rTub = await this.api.postJson({ cells: { [row.openTub.id]: { state: 'Emptied' } } }, 'FlavorTub');
    ...
    const rSlot = await this.api.postJson({ cells: { [row.slotId]: { current_flavor: 0, tubs: 0 } } }, 'Cabinet');
    if (!rSlot.ok || !rSlot.data?.ok) {
