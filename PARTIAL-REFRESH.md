...
- **Existing rows** (matched by row id): classnames and badges are updated in place via `_patchFieldDecorations()`/`_patchRowClasses()`. The writeable FindIt/TextIt control (or read-only text/link) built by `_renderFieldValue()` was originally left completely untouched, so an in-progress edit or open dropdown elsewhere on the page could never be clobbered by someone else's save landing — see §8 below for why that was later loosened for FindIt specifically.
...
A deliberate exception to §4's "never move focus" rule, added afterward for a specific, high-frequency workflow: on `FlavorTub` (and any other grid grouped + filterable via `FindInList`), saving is almost always followed by searching for the *next* flavor to work on, not lingering on the row just saved.
...
Alongside the focus move, the same submit handler also calls `this._filter.clear()` (`find-in-list.js`) — empties the query and re-applies it (so previously-hidden groups reappear) rather than just clearing the input's visible text. Without the re-apply, setting `.value = ""` alone wouldn't fire FindInList's own `input` listener, leaving the box empty but the grid still visually narrowed to whatever was typed before. This puts the grid back to a clean, unfiltered state exactly when focus lands in the box, ready for the next search.

**The tension this raises:** `FindInList` filters against the live DOM on every keystroke (see §4's audit of `find-in-list.js`), so typing a query while the post-save fetch is still in flight (routinely several seconds — every save invalidates the server's bundle cache, so the fetch that follows one's own save is a guaranteed cold miss, see `performance.md` #4) filters against not-yet-fresh data for whichever group the save touched. Locking the UI until that fetch resolves was rejected as worse than the problem. The resolution:
...
## 6. `refreshPageDomain`'s `force` didn't actually force a fresh fetch

Found while chasing a real bug report: BatchHistory's delete button wasn't removing the row from the GUI after a confirmed delete, even though the server-side delete and cache-bust were both confirmed correct. Root cause was in the one function every write path funnels through, not the delete/render pipeline: [`ScoopAPI.refreshPageDomain`](assets/data/scoop-api.js) checked `if (this._domainInflight) return this._domainInflight;` **regardless of `force`** — a `force:true` caller arriving while an earlier fetch was still in flight silently got handed that earlier (possibly stale) promise instead of a fresh one.

This bit BatchHistory specifically because it's normally mounted embedded next to the Batch-creation form and reopened right after a batch save (`ScoopAPI._mountEmbeddedBatchHistory`); that save's own forced refresh is fire-and-forget and can take 10-15s cold. A delete landing while it's still resolving got the pre-delete snapshot. The same race can hit any two overlapping forced refreshes on one page (a CabinetWorkflow write racing another grid's Save, for instance), so the fix is in the shared function: a forced call arriving mid-flight now chains a genuine follow-up fetch onto the in-flight one instead of reusing it, trading a rare extra round-trip for correctness — consistent with why every caller already passes `force:true` in the first place.

## 7. CabinetWorkflow: the additive patch is a structural no-op for hand-built cards, and its native alert() cascade blocked the confirmation refetch

`CabinetWorkflowTile` is the only view in the codebase with a `buildItemDom` that doesn't render off a column list — `CabinetWorkflowGridModel` ships zero columns by design (see change-tub.md), so §2's additive `_patchRefresh` path, which patches existing rows by iterating a model's columns, had nothing to work with. This was already known and accepted (change-tub.md documents it as the reason the tile's own optimistic-repaint/mismatch-diff mechanism exists), but that mechanism only ever repaints the slot(s) the tile's own write just touched — a refresh from any other cause (another tab, another staff member, a different grid's save) left stale slot cards on screen indefinitely. Fixed by giving the model a `rebuildOnRefresh = true` flag (mirror image of Batch/Closeout's `repaintOnRefresh = false` — same idea, opposite direction) that `_onDomainUpdated` now checks alongside the existing `getServerFilterParams` condition, so any refresh this tile receives does a real rebuild instead of a silent no-op.

Separately, both `cabinet-workflow-tile.js` and `confirm-swap-modal.js` still used native, sequential, blocking `alert()` calls — up to 6 in a row after one tub swap — explicitly flagged in their own comments as debug scaffolding, never finished ("a reviewable change log through another GUI element" was raised as a requirement and scoped out). Every blocking dialog also delayed the confirmation refetch that follows, compounding §6's race window. Replaced with the existing `Toast` mechanism (already used by every other write path in the app — autosave, manual Save, BatchHistory's own delete): failure alerts became single `Toast.addMessage({title, message})` calls, and the multi-line informational cascade after a successful swap/Confirm Cabinet was consolidated into one `Toast.addMessage({title, changes})` call using the same `[{sentence}]` list shape `describeFieldChanges` already established — one coherent, dismissible, reviewable notification per action instead of a stack of dialogs.

## 8. A FindIt cell never updated for a domain refresh it didn't cause

Reported as: changing a slot's flavor via CabinetWorkflow didn't repaint that same slot's flavor in the Cabinet ("Flavor Plan") grid, even when both were mounted on the same dock page and the Cabinet grid's own `domain.slot` data had genuinely refreshed underneath it.

Root cause was §2's own trade-off, more literally than intended: `_patchFieldDecorations()` updated a cell's alertCase/badges but was written to *never* touch the FindIt control's actual value, on the theory that only the editing grid needs its value right (already correct from the optimistic `FindIt.select()`/`.update()` that ran when the edit happened) and every other grid just needed decorations to stay current. That's true for the grid that caused the write, but leaves every *other* grid showing the same underlying row (e.g. Cabinet and CabinetWorkflow both rendering `slot`) permanently stale for that field — there was never a path back to a correct value short of a full page reload.

Fixed by having `_patchFieldDecorations()` also call the FindIt's own `refresh(data)` (already existed, unused by this path) — but only when it's actually safe: not this cell's own pending edit (`dirtySet`, keyed `${rowId}|${colKey}` same as `_handleCellChange`) and not literally focused or open right now (`document.activeElement === findIt.INP`, `findIt.isOpen`). `EL._findIt` (tagged in `_renderFieldValue` at build time) is how the patch step finds the live instance to call. TextIt/ToggleIt cells are untouched by this — no reported case needs it yet, and they don't currently expose a `refresh()`-style method to call safely.

## Not yet touched

One other repaint-aggressiveness source identified during this pass is still open:

- **Autosave background refresh** ([_list.js `_scheduleBackgroundDomainRefresh`](assets/ui/_list.js)) — every autosaved field edit still triggers a full page-wide bundle *refetch* 800ms later (now a gentler repaint, thanks to §2/§4 above, but still a network round-trip for every grid on the page for one cell's edit). More precisely: **every** `refreshPageDomain` call, forced or not, always refetches the full page-wide type union (`this._pageTypes`), never just the types actually affected by whatever triggered it — no type-scoping exists today. Tracked as a roadmap item (see project memory) alongside the related server-side cache-bust granularity work in `performance.md` #4/#13, rather than tackled here.

**Closed:** `_bundleCache` (client-side bundle caching) was previously flagged here as dead code pending removal, pointing at "the stashed work (`git stash list`)." Re-checked during this pass: `_bundleCache` doesn't exist anywhere in `assets/` and there are no stashes. Already resolved, nothing further to do.
