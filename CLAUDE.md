...
There is **no build step and no package manager**. JS files are loaded directly as ES modules by the browser (`script_loader_tag` filter rewrites the `scoop-grid` handle to `type="module"`). The test suites under `tests/` (visual, smoke, unit) are dev-only — the deploy rsync excludes them. `tests/unit` runs in CI on every push to `main`, every pull request, and pushes to `worktree-tub-moving` (`.github/workflows/unit-tests.yml`; deliberately no path filter — "runs only when relevant" is how the visual baselines silently went stale), with php-cli installed so the PHP parser suite executes there too. The same workflow's `behavior-tests` job runs the visual suite's `@behavior`-tagged specs (real components, real Chromium, state assertions — no screenshots) on every push/PR, so a client-behavior regression can't ship unseen.
